...
while start:
    ...
    titlelist = tree.xpath(
        '//div[@id="div_more_news"]/div/div/a/@title')  #名称列表
    urllist = tree.xpath('//div[@id="div_more_news"]/div/div/a/@href')  #链接列表
    datelist = tree.xpath(
        '//div[@id="div_more_news"]/div/div[3]/text()')  #日期列表

    table_name = 'tongzhi' + str(datetime.datetime.now()).replace(
        ' ', '').replace('-', '').replace(':', '').replace('.', '')[:14]  #表名
    ...
    with db.cursor() as cursor:
        ...
        for dept in cursor.fetchall():
            print(dept)  #输出
        cursor.execute('insert into tongzhi select * from ' + table_name +
                       ' where title not in (select title from tongzhi)')  #去重
