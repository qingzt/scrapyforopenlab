...
void chain<T>::set(int theIndex,const T& theElement)
{
   ...
   if (theIndex == 0)
   {
      ...
      firstNode = new chainNode<T>(theElement, deleteNode->next);
      delete deleteNode;
   }
   else
   {
      ...
      currentNode->next=new chainNode<T>(theElement, deleteNode->next);
      delete deleteNode;
   }
}
...
{
   chainNode<T> *bef=NULL,*ptr=firstNode,*aft=ptr->next;
