...
for no in range(0, sum):
    every = []#热榜的每一条存放在其中
    title = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].question.title')[0]#获得标题
    type_list = jsonpath.jsonpath(
        obj, '$.data[' + str(no) + '].question.topics[*].name')#获得分类
    topics = ",".join(type_list)#将分类列表转为字符串
    level = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].reaction.score')[0]#获得热度等级
    url = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].question.url')[0]#获得连接
    follow_num = jsonpath.jsonpath(
        obj, '$.data[' + str(no) + '].reaction.follow_num')[0]#获得关注人数
    pv = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].reaction.pv')[0]#获得阅读量
