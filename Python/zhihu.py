import urllib.request
import jsonpath
import json
import pymysql
import datetime


def insert(every):
    db = pymysql.connect(host="127.0.0.1",
                         user="root",
                         password="wssb",
                         database="zhihu")
    cursor = db.cursor()
    SQL = 'insert into ' + table_name + ' values("' + every[0] + '","' + every[
        1] + '",' + str(every[2]) + ',"' + every[3] + '",' + str(
            every[4]) + ',' + str(every[5]) + ',' + str(every[6]) + ',' + str(
                every[7]) + ')'
    cursor.execute(SQL)
    db.commit()
    db.close()


baseurl = 'https://www.zhihu.com/api/v4/creators/rank/hot?domain=0&limit=102&offset=0&period=hour'
headers = {
    'cookie':
    '_zap=4edafd7a-cd56-4669-89f6-fa1628e41200; d_c0=AEATawpdnRWPTiyFWHf13w-E0jTx0w4TpLE=|1664117734; ISSW=1; __snaker__id=YjwhLaRYgKnYrgFQ; gdxidpyhxdE=Qhil%2FGXzqPtE2%2FgAeK3ewnRY1lrW%5CY33%5CvvakJ8rI%2BMOlQUbjQHTK3pYU2BGCq3LPW7jQKDmu9IRUK76uGP2eq8PvK4f%2Ft1P8TuVt0ykzfkOC%2BL0I%5C%2Feml4PeCELH%2FNH85yg7A1cjhT%2FCEJ8kCYQ%2FHZy6EeBeM3rUY5PuDWRQQBuVITN%3A1665847904065; _9755xjdesxxd_=32; YD00517437729195%3AWM_NI=XmGZKbawjCmbzLR85lgg2fPJ%2FKXGl16BnC4aa83n11DyZMdwNVQL0M%2FOmxkaK5bONIFPfUKomoguZZwFgnKpTakrhd909%2FgTQaAIfUAUnKMNETGZ4%2FzEp8GTlcxBaRt6SEc%3D; YD00517437729195%3AWM_NIKE=; YD00517437729195%3AWM_TID=RmSf2so7XFJFFRAQRAfVT%2BLgG8VfAD1m; captcha_session_v2=2|1:0|10:1665847108|18:captcha_session_v2|88:WXhRRWQ5a0JWS3V1YWZWTkJtQUpRbEtKR2VxRHNvUE5WUkZpUG5hd2RkRjY4VC85cTRsZU5PNzFkYzRvZ3hPZg==|f2fd3c577b2b07d0a0e67de24068e934a5abcee010f3093b0147ca55ac847776; captcha_ticket_v2=2|1:0|10:1665847113|17:captcha_ticket_v2|704:|40227a0962ae64fdb0085feae65bafb462ca1c7c0c3a39dc8df9c9ec17f4ed37; z_c0=2|1:0|10:1665847127|4:z_c0|92:Mi4xU3VaOUx3QUFBQUFBUUJOckNsMmRGU1lBQUFCZ0FsVk5WeDA0WkFEUEEyTzdYLWVwM0pTMTU0ZTNPZ05KUmJTYTV3|2723ab34c5f990d0a95c64cc234eacd9bbc4e368a240fef4eaf61f62f0803401; q_c1=9af9e9cc817f42c4b3ebbb173e1fc6eb|1665847175000|1665847175000; tst=h; _xsrf=dc86e7eb-3f47-40cf-8cd9-37a3af08ad2f; SUBMIT_0=68114eff-219c-4e08-9fa7-3f7e8cf159e8; SESSIONID=P3pBvVyaSNdiPl9aV28U3daDXoHdXQY4ny4OJg6hm8q; JOID=U1wWAkgfxCSPTkEYJB_DdVZCHIIzSvBWzzoSZkdOj0fkHC8kQmOub-xJQx0kQScgd7ksvWFbvjj4LhBAytD6u1M=; osd=WlkdAEkWwS-NT0gdLx3CfFNJHoM6T_tUzjMXbUVPhkLvHi4tR2isbuVMSB8lSCIrdbgluGpZvzH9JRJBw9XxuVI=; KLBRSID=fb3eda1aa35a9ed9f88f346a7a3ebe83|1665889061|1665888120',
    'user-agent':
    ' Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36'
}
requset = urllib.request.Request(url=baseurl, headers=headers)
response = urllib.request.urlopen(requset)
content = response.read().decode("utf-8")
with open('rebang.json', 'w', encoding='utf-8') as fp:
    fp.write(content)
obj = json.load(open('rebang.json', 'r', encoding='utf-8'))
title_list = jsonpath.jsonpath(obj, '$..title')
sum = len(title_list)
infor_list = []
for no in range(0, sum):
    every = []
    title = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].question.title')[0]
    type_list = jsonpath.jsonpath(
        obj, '$.data[' + str(no) + '].question.topics[*].name')
    topics = ",".join(type_list)
    level = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].reaction.score')[0]
    url = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].question.url')[0]
    follow_num = jsonpath.jsonpath(
        obj, '$.data[' + str(no) + '].reaction.follow_num')[0]
    pv = jsonpath.jsonpath(obj, '$.data[' + str(no) + '].reaction.pv')[0]
    answer_num = jsonpath.jsonpath(
        obj, '$.data[' + str(no) + '].reaction.answer_num')[0]
    upvote_num = jsonpath.jsonpath(
        obj, '$.data[' + str(no) + '].reaction.upvote_num')[0]
    every.append(title)
    every.append(topics)
    every.append(level)
    every.append(url)
    every.append(follow_num)
    every.append(pv)
    every.append(answer_num)
    every.append(upvote_num)
    infor_list.append(every)
table_name = 'zhihu' + str(datetime.datetime.now()).replace(' ', '').replace(
    '-', '').replace(':', '').replace('.', '')[:14]
createSQL = 'create table ' + table_name + ' select * from zhihu'
db = pymysql.connect(host="127.0.0.1",
                     user="root",
                     password="wssb",
                     database="zhihu")
cursor = db.cursor()
cursor.execute(createSQL)
db.commit()
db.close()
for no in range(0, len(infor_list)):
    insert(infor_list[no])
selectSQL = 'select title as "标题",topics as "分类",level as "热力值",url as "链接",follow_num as "关注人数",pv as "浏览量",answer_num as "回答数",upvote_num as "点赞数" from ' + table_name
db = pymysql.connect(host="127.0.0.1",
                     user="root",
                     password="wssb",
                     database="zhihu")
with db.cursor() as cursor:
    cursor.execute(selectSQL)
    for dept in cursor.fetchall():
        print(dept)
db.close()
